 ...
 * 快速排序法 (Quick Sort)
 *
 * <p>是一種分而治之的演算法 (Divide and Conquer)
 *
 * <p>具體作法是先尋找一個基準值(pivot), 基準值可以隨便找,
 *
 * <p>為了方便這裡使用 arr[0] 當作基準
 *
 * <p>以基準值為準, 將陣列分成 比基準值小 和 比基準值大的陣列
 *
 * <p>分完之後, 進行遞迴, 直到每一陣列長度為1時, 陣列就已經排序好了
 */
  ...
  private void recursive(final int[] ints, final int start, final int end) {
    ...
    if (start < end) {
      ...
      while (true) {
        ...
        while (left < right && ints[left] < pivot) left++;
        ...
        while (right >= left && ints[right] > pivot) right--;
